...
class Context:
    ...
    def __init__(self):
        securityFile = os.path.expanduser(SECURITY_FILE)

        if not os.path.isfile(securityFile):
            raise Exception('could not initialize Digital Ocean Context because the security file, ' + securityFile + 'does not exist' )

        with open(securityFile,'r') as sFile:
            self.securityInfo = json.load(sFile)
            print('loaded security information from ' + securityFile, file=sys.stderr)
